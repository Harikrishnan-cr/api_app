...
class LocalVideoPlayerScreen extends StatefulWidget {
  LocalVideoPlayerScreen({
    ...
    this.videoUrlLocalPath,

    // required this.lessonList,
  });
  int currentIndex;
  String? videoUrlLocalPath;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        children: [
          ...
          _CustomPlayer(
            videoUrlPathLocal: widget.videoUrlLocalPath,
          ),
...
class _CustomPlayer extends StatefulWidget {
  const _CustomPlayer({this.videoUrlPathLocal});
// final LessonMediaModel model;

  final String? videoUrlPathLocal;
...
class _CustomPlayerState extends State<_CustomPlayer> {
  ...
  void _initializePlayer({String? videoUrl}) async {
    ...
    // });
    // String videoPath =
    //     "/data/user/0/com.samastha.elearning/app_flutter/samastha-358083f0-27a6-1e7d-9fc3-bd1d41791c44.mp4";

    if (widget.videoUrlPathLocal != null) {
      //setState(() {
      flickManager = FlickManager(
        videoPlayerController: VideoPlayerController.file(
          File(widget.videoUrlPathLocal!),
        ),
        onVideoEnd: () {
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      aspectRatio: 16 / 9, //if video is in full screen change the aspect ratio
